...
const ProductItem = ({name, price, images, _id, discount, from, to, ...rest}) => {
  ...
  const addToCart = () => {
    dispatch(cartAction.addToCart({name, price, images, _id, discount, from, to, ...rest, quantity: 1}))
  }
  ...
  return (
    <div className="product-item">
      ...
      <div className="product-item__back">
        <div className="overlay-image" onClick={() => navigate(`/product/${_id}`)}>
      ...
      <p className="product-title" onClick={() => navigate(`/product/${_id}`)}>{name}</p>
      <p className="product-item__price">{Intl.NumberFormat().format(newPrice)} VND</p>
